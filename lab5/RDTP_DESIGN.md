# Что такое RDTP и зачем он нужен

RDTP — это учебный протокол, который мы создали, чтобы разобраться, как на практике работает надёжная передача данных поверх ненадёжного UDP.

## Основная идея: UDP сам по себе не гарантирует, что пакеты дойдут по порядку и без потерь. RDTP добавляет эту надёжность поверх UDP, используя механизмы, которые существуют в реальных протоколах вроде TCP.
